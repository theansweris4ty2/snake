...
main :: proc() {
	...
	snake_head := Vec2i{GRID_WIDTH / 2, GRID_WIDTH / 2}
	...
	for !rl.WindowShouldClose() {
		...
		head_rect := rl.Rectangle {
			f32(snake_head.x) * CELL_SIZE,
			f32(snake_head.y) * CELL_SIZE,
			CELL_SIZE,
			CELL_SIZE,
		}
		tail_rect := rl.Rectangle {
			f32(snake_head.x + tail_position.x) * (CELL_SIZE) + 1,
			f32(snake_head.y + tail_position.y) * CELL_SIZE,
			CELL_SIZE,
			CELL_SIZE,
		}
		tail2_rect := rl.Rectangle {
			f32(snake_head.x + (tail_position.x * 2)) * (CELL_SIZE) + 1,
			f32(snake_head.y + (tail_position.y * 2)) * CELL_SIZE,
			CELL_SIZE,
			CELL_SIZE,
		}
